...
#define JOYSTICK_VENDOR_ID   0x1a86
#define JOYSTICK_PRODUCT_ID  0x7523
...
{
	std::system("clear");
	for(int i = 0; i < 32; ++i)	qDebug() << i << ": " << Qt::hex << data_ch[i] << Qt::endl;
//	std::system("clear");
}
...
//		{			result = libusb_interrupt_transfer(m_device, 0x81, data_ch, sizeof(data_ch), &bytesTransferred, 10);
		...
		if(result == LIBUSB_SUCCESS)	
		{
			qDebug() << "successfuly received data " << sizeof(data_ch) << '\n';
			printRawData();
		}
//		});
